...
public class OrderAcceptanceTest extends AcceptanceTest {
    ...
    @DisplayName("주문을 한다.")
    void orderSuccess() {
        final var response = 주문_결제_요청(PointFixture.멤버2_ID, 감자_UUID);

        assertAll(
                () -> assertThat(response.statusCode()).isEqualTo(200),
                () -> assertThat(포인트_잔액_확인(PointFixture.멤버2_ID)).isEqualTo(5000L),
    ...
    void orderFailSoExpensive() {
        다중_주문_결제_요청(PointFixture.멤버2_ID, 감자_UUID, 2);
        final var response = 주문_결제_요청(PointFixture.멤버2_ID, 당근_UUID);

        assertAll(
                () -> assertThat(response.statusCode()).isEqualTo(400),
                () -> assertThat(포인트_잔액_확인(PointFixture.멤버2_ID)).isEqualTo(0)
        );
    }
    ...
    void orderNotExistMenu() {
        final var response = 주문_결제_요청(PointFixture.멤버2_ID, "notExistMenuUUID");
